...
| [v35_patent_version.md](v35_patent_version.md) | V35 design/status record: exact flags, restored config, loss recipe, the two historical launch faults, and the known-bug list the code-fix backlog is built on. |
| [patent_draft.md](patent_draft.md) | Patent draft. ⚠ Written against v37 with **SVFW as claim #1**; V35 does not use SVFW, so filing on this line requires promoting the multi-scale cross-attention (today's claim #2) to claim #1 — see v35_patent_version.md §3. |
